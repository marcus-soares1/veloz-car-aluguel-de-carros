...
import { IVehicle, ICreateVehicle, IVehicleRepository } from "../interfaces/IVehiclesRepository"
...
describe('vehiclesRepository', () => {
    ...
    describe('update', () => {
        it('should update a vehicle', async () => {
            const updatedVehicle = { ...mockVehicle, color: "Branco" };
            (prisma.vehicles.update as jest.Mock).mockResolvedValue(updatedVehicle);
            const result = await repository.update(1, { color: "Branco" });
        ...
        it('should return null if update vehicle not found', async () => {
            ...
            const result = await repository.update(999, { color: "Branco" } );
            expect(result).toBeNull();
        });
    });
